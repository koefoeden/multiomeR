...
The module config controls which variables are modelled, which metadata columns are used, and which contrasts are run. Defaults and validation rules for module parameters live in `cfg_pipeline_parameters.tsv`. Edit `module_differential_analyses/cfg.yaml`, keeping disabled or example rows available for reference until the corresponding aggregation is ready.
