...
A small number of R packages still need GitHub versions, since they are not currently available on conda-forge/bioconda. The `pixi run install-r-github-packages` task installs the pinned GitHub versions of BPCells, Signac, and betterChromVAR into the pixi R library.
:::
...
Then download and unpack the matching human Cell Ranger ARC reference (14.9 GB) (or simply symlink it to example_data/refdata-cellranger-arc-GRCh38-2024-A if you have one).
