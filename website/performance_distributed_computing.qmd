...
multiomeR runs targets through the [targets](https://books.ropensci.org/targets/crew.html) and `crew` backend. The repository ships with `crew_controllers.R` pointing to a local-controller setup, which is sufficient for the public PBMC demo.

For larger datasets, adapt `crew_controllers.R` to match your scheduler, queue names, resource limits, and worker startup requirements.
