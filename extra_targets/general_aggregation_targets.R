rlang::list2(
  ...
  tarchetypes::tar_file(
    name = scE2G_ATAC_candidate_region_BEDs_per_cluster,
    description = "Write per-cluster ATAC candidate regulatory region TSV files for scE2G input",
    command = {
      file_out <- get_structured_file_path(
        filetype = "tsv",
        override_suffix = peak_calling_cluster_names.ATAC
      )
      readr::write_tsv(
        dplyr::select(
          tibble::as_tibble(within_clusters_collapsed_peaks_per_cluster_GRanges.ATAC),
          1:3
        ),
        file = file_out,
        col_names = FALSE
  ...
  tarchetypes::tar_file(
    name = scE2G_GEX_count_matrix_files,
    description = "Extract per-cluster GEX count matrices as compressed CSVs for scE2G input",
    command = {
      file_out <- get_structured_file_path(
        filetype = "csv.gz",
        override_suffix = peak_calling_cluster_names.ATAC
      ...
      keep_BCs <- intersect(
        colnames(aggregated_counts_BPCells_matrix.GEX),
        BCs_per_peak_cluster_list.ATAC
      )

      readr::write_csv(
        tibble::rownames_to_column(
          as.data.frame(as.matrix(aggregated_counts_BPCells_matrix.GEX[,
            keep_BCs,
            drop = FALSE
          ])),
          var = "gene"
  ...
  tarchetypes::tar_file(
    name = scE2G_clusters_tibble,
    description = "Write the scE2G cluster configuration TSV file linking cluster names to input data paths",
    command = {
      clusters_tibble_path <- get_structured_file_path(filetype = "cfgtsv")

      readr::write_tsv(
        tibble::tibble(
          "cluster" = paste0(
            tar_name_wo_suffixes(),
            "_",
            unlist(peak_calling_cluster_names.ATAC)
          ),
          "rna_matrix_file" = fs::path_abs(unlist(
            scE2G_GEX_count_matrix_files
          )),
          "atac_frag_file" = fs::path_abs(unlist(fragments_per_cluster.fragments.ATAC)),
          "atac_peaks_file" = fs::path_abs(unlist(
            peaks_per_cluster_narrowPeaks.peaks.ATAC
          )),
          "atac_candidate_region_BEDs" = fs::path_abs(unlist(
            scE2G_ATAC_candidate_region_BEDs_per_cluster
          )),
          "HiC_file" = NA,
          "HiC_type" = NA,
          "HiC_resolution" = NA,
        ...
        file = clusters_tibble_path
      )

      clusters_tibble_path
    }
  )
)
