rlang::list2(
  targets::tar_target(
    ...
    description = "Sum full QC metadata barcode counts across all reactions in this dataset",
    command = sum(unlist(dataset_unfiltered_cells_n_vecs_syms)),
  ),
  targets::tar_target(
    name = per_dataset_excluded_BCs_list,
    description = "Combine per-reaction QC exclusion lists into a single dataset-level list",
  ...
  tarchetypes::tar_file(
    ...
    description = "Plot an UpSet plot of dataset-level QC exclusion overlaps across all reactions and save to file",
  ...
  targets::tar_target(
    ...
    description = "Combine per-reaction CellRanger-only QC exclusion lists into a dataset-level list",
    command = collapse_duplicate_names(purrr::flatten(
  ...
  targets::tar_target(
    ...
    description = "Combine CellRanger-kept metadata across reactions, selecting QC variables for per-dataset QC plots",
  ...
  tarchetypes::tar_file(
    ...
    description = "Plot violin plots of QC metrics per reaction for this dataset and save to file",
    command = {
      threshold_tibble <- get_QC_exclude_threshold_tibble(
        QC_exclude_vector = dataset_QC_exclude_list_per_reaction,
        feature_names = PROCESSING_PER_REACTION_QC_VARS
      )

      plot_data <- per_dataset_cellranger_kept_metadata_tibble |>
        tidyr::pivot_longer(
          cols = dplyr::any_of(PROCESSING_PER_REACTION_QC_VARS),
          names_to = "feature",
          values_to = "value"
        ) |>
        dplyr::filter(
