...
|---|---|---|---|---|
| A1 | reject | `website/demo_installation.qmd` | Fresh quickstart may load the tracked Esrum/SLURM `crew_controllers.R` before users choose a local controller. | The committed symlink already points to the local template; documentation now explains how to create a machine-specific file. |
...
| A5 | accept | `website/main_inputs.qmd` | Aggregations default to active, so template rows can accidentally enter unqualified runs. | Added an explicit active-scope warning and narrow-run guidance. |
...
| C7 | accept | `website/performance_distributed_computing.qmd` | Local template guidance is too optimistic for 60 GB machines. | Added template sizing and safe-concurrency guidance. |
...
| D4 | accept | `website/downstream_differential_analyses.qmd` | Module config page says edit symlinked/template config directly. | Added project-specific config guidance. |
