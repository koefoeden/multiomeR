...
#'   `GEM_well_count`, and `cellranger_input_nuclei` columns. If `NULL`, derive
...
estimate_multimodal_seurat_walltime <- function(
  ...
  if (is.null(aggregation_metadata)) {
    ...
    required_columns <- c("aggregation", "GEM_well_count", "cellranger_input_nuclei")
    ...
    if (
      !is.numeric(aggregation_metadata$GEM_well_count) ||
        !is.numeric(aggregation_metadata$cellranger_input_nuclei) ||
        anyNA(aggregation_metadata[c("GEM_well_count", "cellranger_input_nuclei")]) ||
        any(aggregation_metadata$GEM_well_count <= 0) ||
        any(aggregation_metadata$cellranger_input_nuclei <= 0)
    ) {
      stop(
        "aggregation_metadata GEM well and nuclei counts must be positive numeric values.",
        call. = FALSE
      )
    }
    GEM_well_counts <- stats::setNames(
      aggregation_metadata$GEM_well_count,
