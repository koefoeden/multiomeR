...
1. `GEM_well_tibble_all` reads every row and its pre-aggregation processing settings from `cfg_GEM_wells.tsv`.
...
5. `dataset_tibble` groups those active GEM well rows by the `dataset` label for QC summaries.
6. `_targets.R` expands active GEM wells, derived dataset summaries, and aggregations with `tar_map()`, then appends module target files.
