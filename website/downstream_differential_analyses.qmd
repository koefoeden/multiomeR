...
your_aggregation:
  differential_analyses_DCTC_models:
    condition_abundance:
      formula: cbind(n_nuclei, n_other_nuclei) ~ condition
      contrast_specs_vec:
...
Both branches use named models, donor eligibility checks and named contrasts. Abundance models use `differential_analyses_DCTC_models`; feature models use `differential_analyses_psbulk_DX_models`. Omitting abundance models disables that branch. The former aggregation-wide DCTC formula, phenotype and colour settings have been replaced by fields inside each named model.
