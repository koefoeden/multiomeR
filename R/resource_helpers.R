...
get_tar_resources <- function(
  ...
  if (is.null(controller_resources)) {
    if (!exists("bootstrap_state_env", inherits = TRUE) || is.null(bootstrap_state_env$controller_resources_tibble)) {
      stop("Controller resources are not loaded. Call load_project_runtime() first.", call. = FALSE)
    }
    controller_resources <- bootstrap_state_env$controller_resources_tibble
  }
